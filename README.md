...
Agent.act ретраит с _CORRECTION, а после 3 попыток ставит случайное число (agent.py:113) — раунд доигрывается; - ложить

проверить, почему gpt-oss не работает

прогнать на разных моделях

описать, как работает

поправить промпты

сделать персонализацию опциональной

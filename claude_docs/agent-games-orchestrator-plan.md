...
class AgentSpec:        persona: str; provider: ProviderCfg; count: int = 1
@dataclass(frozen=True)
class PopulationCfg:    kind: str; agents: list[AgentSpec]   # размер = sum(spec.count)
@dataclass(frozen=True)
class EpisodeCfg:
...
class RosterGenerator:
    ...
    def build(self, rng) -> Population               # каждый spec раскрыт по count -> A1..An
...
`RosterGenerator.build`: `for spec in agents: for _ in range(spec.count):
pop.add(AgentSetup(spec.persona, spec.provider))` → ID `A1..An` (агенты сгруппированы по
типу), провайдеры из кэша.
...
- Тесты `test_roster.py`: каждая специя множится на свой `count`, ID `A1..An`; **один провайдер на

...
- **Формат записи** (с июня 2026 — игровой транскрипт, не сводка): каждый прошлый раунд
  ...
  `<you>` (свои), `<Имя>` (партнёра). Блок раунда: открытие чата (кто говорил первым —
  видно по порядку реплик), реплики cheap-talk, закрытие чата (с причиной —
  ...
  (`history_*`, `msg_*`, `reason_*`), не в коде; `render_turns` (в `memory.py`)
  ...
  DECIDE). Каждый промпт — цельный шаблон, без отдельной вставки «кто открыл»: порядок
  реплик в транскрипте это и так показывает. Пример блока:
  ```
